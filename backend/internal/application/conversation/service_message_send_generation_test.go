...
func TestGenerationCallResponseHandleMarksProviderEffectAndRecoveryMetadata(t *testing.T) {
	service := &Service{generationStreams: newGenerationStreamRegistry(nil, generationStreamOptions{})}
	runner := &messageGenerationRunner{
		service:                     service,
		runID:                       "run_response_handle",
	...
	if err := call.handleStreamEvent(context.Background(), input, llm.GenerateStreamEvent{ResponseID: "resp_current"}); err != nil {
		t.Fatalf("handle response handle: %v", err)
	}
	if !runner.attemptHadSideEffect || !call.observation.providerEffectObserved {
		t.Fatal("a current-attempt response handle must block unsafe retry provenance")
	}
	if runner.responsesBackgroundRecovery.ResponseID != "resp_current" {
		t.Fatalf("expected current response handle for recovery, got %q", runner.responsesBackgroundRecovery.ResponseID)
	}
	if runner.responsesBackgroundRecovery.ResponseID == input.PreviousResponseID {
		t.Fatal("input previous_response_id must not be mistaken for the current response handle")
	}
	knownRejection := llm.MarkRequestAccepted(&llm.UpstreamError{
		StatusCode: 400,
		Message:    "stream is not supported by this model",
	})
	if call.observation.canRetry(knownRejection, shouldFallbackToNonStreaming) {
		t.Fatal("known shape rejection cannot retry after a current response handle")
	}
}
