...
func toConversationRunDomain(item models.ConversationRun) domainconversation.Run {
	return domainconversation.Run{
		...
		TriggerCreatedAt:         item.TriggerCreatedAt,
		ConversationID:           item.ConversationID,
...
func toFileObjectDomain(item models.FileObject) domainconversation.FileObject {
	return domainconversation.FileObject{
		...
		FileID:                        item.FileID,
		UserID:                        item.UserID,
		ProcessingTriggererUserID:     item.ProcessingTriggererUserID,
		ProcessingResourceOwnerUserID: item.ProcessingResourceOwnerUserID,
		ProcessingPurpose:             item.ProcessingPurpose,
		ProcessingRunID:               item.ProcessingRunID,
		ProcessingExecutionID:         item.ProcessingExecutionID,
		ProcessingParentExecutionID:   item.ProcessingParentExecutionID,
		ProcessingTriggerCreatedAt:    item.ProcessingTriggerCreatedAt,
		Purpose:                       item.Purpose,
		FileName:                      item.FileName,
		MimeType:                      item.MimeType,
...
func toFileObjectProcessingStateDomain(item models.FileObject) domainconversation.FileObjectProcessing {
	return domainconversation.FileObjectProcessing{
		...
		UserID:              item.UserID,
		TriggererUserID:     item.ProcessingTriggererUserID,
		ResourceOwnerUserID: item.ProcessingResourceOwnerUserID,
		Purpose:             item.ProcessingPurpose,
		RunID:               item.ProcessingRunID,
		ExecutionID:         item.ProcessingExecutionID,
		ParentExecutionID:   item.ProcessingParentExecutionID,
		TriggerCreatedAt:    item.ProcessingTriggerCreatedAt,
		DetectedMIME:        item.DetectedMIME,
		FileCategory:        item.FileCategory,
		ProcessingStatus:    item.ProcessingStatus,
		ProcessingReady:     item.ProcessingReady,
...
func fileObjectProcessingStateUpdates(item *domainconversation.FileObjectProcessing) map[string]any {
	...
	// Older workers build state DTOs without the new envelope. Do not let such a
	// legacy/local update erase an already persisted actor or operation reference.
	// A producer that intentionally starts a new operation supplies at least one
	// trusted metadata field and updates the complete immutable set below.
	if fileObjectProcessingHasOperationMetadata(item) {
		updates["processing_triggerer_user_id"] = item.TriggererUserID
		updates["processing_resource_owner_user_id"] = item.ResourceOwnerUserID
		updates["processing_purpose"] = strings.TrimSpace(item.Purpose)
		updates["processing_run_id"] = strings.TrimSpace(item.RunID)
		updates["processing_execution_id"] = strings.TrimSpace(item.ExecutionID)
		updates["processing_parent_execution_id"] = strings.TrimSpace(item.ParentExecutionID)
		if item.TriggerCreatedAt != nil {
			updates["processing_trigger_created_at"] = item.TriggerCreatedAt
		}
	}
	return updates
}

func fileObjectProcessingHasOperationMetadata(item *domainconversation.FileObjectProcessing) bool {
	if item == nil {
		return false
	}
	return item.TriggererUserID != 0 ||
		item.ResourceOwnerUserID != 0 ||
		strings.TrimSpace(item.Purpose) != "" ||
		strings.TrimSpace(item.RunID) != "" ||
		strings.TrimSpace(item.ExecutionID) != "" ||
		strings.TrimSpace(item.ParentExecutionID) != "" ||
		item.TriggerCreatedAt != nil
}
